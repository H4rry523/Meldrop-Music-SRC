...
bot = commands.Bot(command_prefix=get_prefix,description='A music bot made by an Anonymous user in Philippines!')
...
async def play(con, *, url):
    ...
    if con.message.channel.is_private == True:
        await bot.send_message(con.message.channel, "```You must be in a valid server text channel` to use this command!```")
...
async def skip(con):
    ...
    if con.message.channel.is_private == False:
        if servers_songs[con.message.server.id] == None or len(song_names[con.message.server.id]) == 0 or player_status[con.message.server.id] == False:
            await bot.send_message(con.message.channel, "```No songs in queue to skip, add songs in the queue to skip!```")
        if servers_songs[con.message.server.id] != None:
            await queue_songs(con, True, False)

@bot.command(pass_context=True)
async def join(con,*,channel=None):
    """JOIN A VOICE CHANNEL THAT THE USER IS IN OR MOVE TO A VOICE CHANNEL IF THE BOT IS ALREADY IN A VOICE CHANNEL"""
    ...
    if con.message.channel.is_private == False:
        ...
        if voice_status == False and channel == None:  # VOICE NOT CONNECTED
            if con.message.author.voice_channel == None:
                await bot.send_message(con.message.channel,"```You must be in a voice channel or a valid voice channel name to join!```")
            if con.message.author.voice_channel != None:
                await bot.join_voice_channel(con.message.author.voice.voice_channel)
        ...
        if voice_status == True:  # VOICE ALREADY CONNECTED
            if voice == None:
                await bot.send_message(con.message.channel, "```Bot is already connected to a voice channel, no need to spam the command!```")
...
async def leave(con):
    ...
    # COMMAND USED IN DM
    if con.message.channel.is_private == True:
        await bot.send_message(con.message.channel, "```You must be in a valid text channel to use this command!```")
    ...
    if con.message.channel.is_private == False:
        if servers_songs[con.message.server.id] != None:
            if paused[con.message.server.id] == True:
                await bot.send_message(con.message.channel, "```Audio already paused, no need to worry!```")
...
async def resume(con):
    # COMMAND IS IN DM
    if con.message.channel.is_private == True:
        await bot.send_message(con.message.channel, "```You must be in a server voice channel to use this command, You dummy!```")
...
async def volume(con,vol:float):
    if player_status[con.message.server.id] == False:
        await bot.send_message(con.message.channel,"```No audio is playing at the moment.```")
    if player_status[con.message.server.id] == True:
